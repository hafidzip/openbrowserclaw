...
const TabRow = memo((
    ...
    return (
        <TableRow className="border-accent/5 hover:bg-accent/5 transition-colors cursor-pointer h-12 group">
            <TableCell className="w-10 cursor-default align-middle" onClick={e => e.stopPropagation()}>
                <Checkbox checked={isSelected} onCheckedChange={handleToggle} />
            </TableCell>
            <TableCell onClick={handleOpen} className="w-8 text-xs text-muted-foreground align-middle">
                <TabIcon iconVal={tab.icon} />
            </TableCell>
            <TableCell onClick={handleOpen} className="max-w-[400px] align-middle">
                <div className="flex items-center gap-2 py-1">
                    {/* Title takes what it needs up to a point, URL fills the rest */}
                    <span className="font-medium text-xs truncate shrink-0 max-w-[160px]">
                        {tab.title || "Untitled"}
                    </span>
                    <span className="text-xs text-muted-foreground opacity-60 truncate flex-1">
                        {tab.url || tab.id}
                    </span>
                </div>
            </TableCell>
            <TableCell onClick={handleOpen} className="text-[11px] text-muted-foreground whitespace-nowrap align-middle pr-4">
...
export default function SiteRegistry({
    ...
    const loadTabs = useCallback(async (pageNum: number, reset: boolean) => {
        ...
        try {
            ...
            const res = await pyInvoke("sqlite", {
                db,
                command: "query",
                sql: `SELECT id, metadata FROM site_registry ${searchClause} ORDER BY rowid DESC LIMIT ${limit} OFFSET ${offset}`,
                params: q ? [`%${q}%`] : []
            });
    ...
    const handleDelete = useCallback(async (id?: string) => {
        ...
        try {
            ...
            await pyInvoke("sqlite", {
                db,
                command: "execute",
                sql: `DELETE FROM site_registry WHERE id IN (${placeholders})`,
                params: ids
            });
            setSelectedIds(prev => {
                const next = new Set(prev);
    ...
    const openTab = (id: string) => {
        ...
        if (tab) {
            const childId = generateIdFromString(tab.id);
